...
class UserDB:
    ...
    def add_user(self, fname: str, lname: str, email: str, unhashed_password: str) -> None:
        hashed_password, salt = hash_password(unhashed_password)
        user = {'fname': fname, 'lname': lname, 'email': email.lower(), 'hashed_password': hashed_password, 'salt': salt}
        self.col.insert_one(user)

    def edit_user(self, email: str, **kwargs) -> None:
        """
        Edit a user's details given their email
        Kwargs can contain: fname, lname, unhashed_password, salt
        """
